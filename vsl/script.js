...
document.addEventListener("DOMContentLoaded", function() {
    ...
    setTimeout(() => {
        ...
        // B. Desliza suavemente até o card de 6 potes
        const card6 = document.getElementById('card-6-bottles');
        if (card6) {
            card6.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }

        // C. Os cards dos potes aparecem 10 segundos após a seção ser revelada
        setTimeout(() => {
            ...
            // D. Inicia os pop-ups de vendas apenas após os cards aparecerem
            startAllNotifications();

        }, 10000); // 10 segundos após o delay principal
    ...
    function startAllNotifications() {
        ...
        // Mostra a primeira notificação 2 segundos após os cards aparecerem
        setTimeout(() => {
            showNotification();
            startRandomInterval();
        }, 2000);
