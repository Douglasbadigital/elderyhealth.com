...
window.addEventListener("message", function(event) {
    ...
    // Listener via postMessage (método principal VTurb)
    window.addEventListener("message", function(event) {
        try {
            var data = typeof event.data === "string" ? JSON.parse(event.data) : event.data;
            if (
                data.type === "smartplayer:timeupdate" ||
                data.event === "timeupdate" ||
                (data.currentTime !== undefined)
            ) {
                var tempo = data.currentTime || data.time || 0;
    ...
    // Fallback: lê o currentTime direto do elemento do player a cada 1 segundo
    var intervalo = setInterval(function() {
        var player = document.getElementById("vid-69d713ad397625cd1e9ad0a8");
        if (player && player.currentTime >= DELAY_SEGUNDOS) {
            mostrarOferta();
