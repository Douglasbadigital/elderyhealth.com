...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Aguarda o player VTurb estar pronto e monitora o tempo do vídeo
    var tentativas = 0;
    var verificar = setInterval(function() {
        tentativas++;
        var player = document.getElementById("vid-69d713ad397625cd1e9ad0a8");
        console.log("Tentativa " + tentativas, player, player ? player.currentTime : "sem player");

        if (player && player.currentTime !== undefined) {
            player.addEventListener("timeupdate", function() {
                console.log("Tempo:", player.currentTime);
                if (player.currentTime >= DELAY_SEGUNDOS) {
                    mostrarOferta();
